...
public sealed class SqlServerImportFileRepository(string connectionString) : IImportFileRepository
{
    ...
    {
        await using var connection=new SqlConnection(connectionString); await connection.OpenAsync(cancellationToken);
        await using var command=new SqlCommand("INSERT dbo.import_files(import_batch_id,import_profile_id,original_file_name,source_sha256,size_bytes) OUTPUT INSERTED.import_file_id VALUES(@batch,@profile,@name,@hash,@size)",connection);
        command.Parameters.AddWithValue("@batch",file.BatchId); SqlServerImportBatchRepository.Add(command,"@profile",file.ImportProfileId); command.Parameters.AddWithValue("@name",file.OriginalFileName); command.Parameters.AddWithValue("@hash",NormalizeHash(file.SourceSha256)); command.Parameters.AddWithValue("@size",file.SizeBytes);
...
public sealed class SqlServerTransactionalImportStore(string connectionString) : ITransactionalImportStore
{
    ...
    private static async Task<long> InsertFile(SqlConnection c,SqlTransaction t,ImportFileRegistration x,CancellationToken token){await using var q=Cmd(c,t,"INSERT dbo.import_files(import_batch_id,import_profile_id,original_file_name,source_sha256,size_bytes) OUTPUT INSERTED.import_file_id VALUES(@batch,@profile,@name,@hash,@size)");q.Parameters.AddWithValue("@batch",x.BatchId);Add(q,"@profile",x.ImportProfileId);q.Parameters.AddWithValue("@name",x.OriginalFileName);q.Parameters.AddWithValue("@hash",SqlServerImportFileRepository.NormalizeHash(x.SourceSha256));q.Parameters.AddWithValue("@size",x.SizeBytes);return Convert.ToInt64(await q.ExecuteScalarAsync(token));}
