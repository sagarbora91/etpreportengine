    ...
    IManagementTrendQuery
{
    ...
    public static ServiceSalesRecord Map(ServiceSalesRow row) => new(row.Period, row.StoreCode, row.PeriodStart, row.PeriodEnd, row.Cash, row.Card, row.Upi, row.Total, row.LastYearTotal, row.GrowthPercent, row.Availability, row.MissingDays, row.LastYearMissingDays);
