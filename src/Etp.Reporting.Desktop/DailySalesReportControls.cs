...
public sealed class KpiCard : Border
{
    ...
    {
        Background = DsrUi.Brush("#FFFFFF"); BorderBrush = DsrUi.Brush("#D9E3E3"); BorderThickness = new Thickness(1); CornerRadius = new CornerRadius(12); Margin = new Thickness(4);
...
public sealed class StorePeriodCard : Border
{
    ...
    {
        Background = DsrUi.Brush("#FFFFFF"); BorderBrush = DsrUi.Brush("#D9E3E3"); BorderThickness = new Thickness(1); CornerRadius = new CornerRadius(12); Margin = new Thickness(4); Padding = new Thickness(10);
...
public sealed class OperationalMetricTable : Border
{
    public OperationalMetricTable(DsrStoreCard store)
    {
        ...
        root.Children.Add(grid); Child = root; Background = DsrUi.Brush("#FFFFFF"); BorderBrush = DsrUi.Brush("#D9E3E3"); BorderThickness = new Thickness(1); CornerRadius = new CornerRadius(12); Padding = new Thickness(12); Margin = new Thickness(4);
    }
...
public sealed class ServiceSummaryCard : Border
{
    ...
    {
        Background = DsrUi.Brush("#FFFFFF"); BorderBrush = DsrUi.Brush("#D9E3E3"); BorderThickness = new Thickness(1); CornerRadius = new CornerRadius(12); Padding = new Thickness(12); Margin = new Thickness(4);
        ...
        var periods = new UniformGrid { Columns = 6, Margin = new Thickness(0, 8, 0, 0) }; foreach (var period in new[] { "FTD", "LY FTD", "MTD", "LY MTD", "YTD", "LY YTD" }) { var cell = new StackPanel(); cell.Children.Add(DsrUi.Text(period, 9, colour: "#65757A", align: TextAlignment.Center)); cell.Children.Add(DsrUi.Text(DsrDisplay.Number(service.PeriodTotals.GetValueOrDefault(period), 0), 11, period is "FTD" or "MTD" or "YTD" ? FontWeights.SemiBold : FontWeights.Normal, period is "MTD" or "YTD" ? "#008D78" : "#10252D", TextAlignment.Center)); periods.Children.Add(cell); } root.Children.Add(periods); Child = root;
    }
}
...
public sealed class DailySalesReportView : Grid
{
    public DailySalesReportView(DailySalesReportDocument report)
    {
        ...
        Background = DsrUi.Brush("#F3F7F7"); Margin = new Thickness(10, 4, 10, 8); RowDefinitions.Add(new RowDefinition { Height = GridLength.Auto }); RowDefinitions.Add(new RowDefinition { Height = GridLength.Auto }); RowDefinitions.Add(new RowDefinition { Height = GridLength.Auto }); RowDefinitions.Add(new RowDefinition { Height = GridLength.Auto }); RowDefinitions.Add(new RowDefinition { Height = GridLength.Auto }); RowDefinitions.Add(new RowDefinition { Height = GridLength.Auto });
        ...
        var kpis = new UniformGrid { Columns = 6 }; kpis.Children.Add(new KpiCard("COMBINED FTD", DsrDisplay.Currency(report.CombinedFtd), $"{DsrDisplay.Percent(report.CombinedFtdGrowth)} vs LY", "#008D78")); kpis.Children.Add(new KpiCard("UNITS", DsrDisplay.Number(report.Units, 0), report.Units is null ? "Data not available" : $"Titan {DsrDisplay.Number(report.Stores[0].Periods[0].TyQuantity, 0)} · Helios {DsrDisplay.Number(report.Stores[1].Periods[0].TyQuantity, 0)}", "#2B7A8B")); kpis.Children.Add(new KpiCard("WALK-INS", DsrDisplay.Number(report.WalkIns, 0), report.WalkIns is null ? "Data not available" : $"Titan {DsrDisplay.Number(report.Stores[0].FtdWalkIns, 0)} · Helios {DsrDisplay.Number(report.Stores[1].FtdWalkIns, 0)}", "#315EBA")); kpis.Children.Add(new KpiCard("CONVERSION", DsrDisplay.Percent(report.Conversion).TrimStart('+'), report.Conversion.Value is null ? "Data not available" : $"{DsrDisplay.Number(report.CombinedInvoices, 0)} invoices / {DsrDisplay.Number(report.WalkIns, 0)} walk-ins", "#087C68")); kpis.Children.Add(new KpiCard("MTD SALES", DsrDisplay.CompactCurrency(report.MtdSales), $"{DsrDisplay.Percent(report.MtdTargetAchievement).TrimStart('+')} of target", "#008D78")); kpis.Children.Add(new KpiCard("YTD SALES", DsrDisplay.CompactCurrency(report.YtdSales), $"{DsrDisplay.Percent(report.YtdGrowth)} vs LY YTD", "#087C68")); Add(kpis, 1);
