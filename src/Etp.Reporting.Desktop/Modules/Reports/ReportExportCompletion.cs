...
public partial class ReportsWorkspaceView
{
    ...
    internal async Task ExportReportToPathAsync(string path, bool pdf)
    {
        ...
        try
        {
            if (pdf) await exportCoordinator.ExportReportPdfAsync(path, report.ExportMetadata!, report.ExportData!, report.VisualReport, report.DailySalesReport);
            else await exportCoordinator.ExportReportExcelAsync(path, report.ExportMetadata!, report.ExportData!, report.VisualReport);
