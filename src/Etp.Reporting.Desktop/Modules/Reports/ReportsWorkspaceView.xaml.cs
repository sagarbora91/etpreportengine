...
public partial class ReportsWorkspaceView : UserControl
{
    ...
    public void ExportExcel()
    {
        ...
        try { exportCoordinator.ExportReportExcel(dialog.FileName, report.ExportMetadata, report.ExportData!, report.VisualReport); ReportResult.Text = $"Excel report saved to {dialog.FileName}"; _ = auditRecorder("ExportExcel", "Succeeded", "Visual report exported"); }
    ...
    private void ExportExcel_Click(object sender, RoutedEventArgs e) => ExportExcel();
    ...
    private bool BeginReportLoad(string reportCode)
    {
        ...
        ExportExcelButton.IsEnabled = ExportPdfButton.IsEnabled = false;
    ...
    private async Task RunStockInventoryAsync(string mode)
    {
        try
        {
            ...
            ApplyReportFilter(); await auditRecorder("ReportRun", rows.Count == 0 ? "Blocked" : "Succeeded", mode == "BRAND" ? "Brand stock" : mode == "SLOW" ? "Slow stock" : "Closing stock");
        }
    ...
    private async Task RunDsrAsync()
    {
        ...
        catch (Exception ex) { ReportResult.Text=$"DSR failed: {ex.Message}"; dailySalesFailure(ex.Message); }
    }

    private async Task RunStaffPerformanceAsync()
    {
        try { var result=await operationalReportQueryFactory(connectionStringProvider()).LoadStaffPerformanceAsync(ReportScope()); ReportGrid.ItemsSource=result.Rows; ReportResult.Text=$"{result.Status}: canonical {result.CanonicalSales:N2}, attributed {result.AttributedSales:N2}, variance {result.Variance:N2}. {result.Message}"; SetExport("Staff CRO Performance",ToReportingStatus(result.Status),result.MetricPolicy,result.Message,[new("Store"),new("CRO"),new("Net Sales","#,##0.00"),new("LY Sales","#,##0.00"),new("Growth %","#,##0.00"),new("Growth Status"),new("Net Quantity","#,##0.00"),new("Discount","#,##0.00"),new("Transactions","#,##0"),new("UPT","#,##0.00"),new("ATV","#,##0.00"),new("Contribution %","#,##0.00"),new("Target","#,##0.00"),new("Achievement %","#,##0.00"),new("Rank","#,##0")],result.Rows.Select(x=>(IReadOnlyList<object?>)[x.StoreCode,x.CroNumber,x.NetSales,x.LastYearSales,x.GrowthPercent,x.GrowthStatus,x.NetQuantity,x.Discount,x.Transactions,x.Upt,x.Atv,x.ContributionPercent,x.TargetSales,x.TargetAchievementPercent,x.Rank]).ToArray(),["Control","",result.AttributedSales,"","","","","",result.Rows.Sum(x=>x.Transactions),"","",result.Variance,"","",""]); ApplyReportFilter(); await auditRecorder("ReportRun",result.Status==ApplicationReportStatus.Passed?"Succeeded":"Failed","Staff performance"); }
