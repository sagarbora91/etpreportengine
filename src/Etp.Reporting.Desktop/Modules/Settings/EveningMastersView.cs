...
public sealed class EveningMastersView : UserControl
{
    ...
    private readonly Func<string> connection;private readonly Func<bool> owner;private int rowId;
    public EveningMastersView(Func<string> connection,Func<bool> owner)
    {
        ...
        this.connection=connection;this.owner=owner;store.Text="WLMHW";order.Text="10";
    ...
    private async Task Run(Func<Task> action){try{if(!owner())throw new UnauthorizedAccessException("Owner permission is required.");IsEnabled=false;await action();}catch(Exception ex){status.Text=DesktopFriendlyError.Describe(ex);}finally{IsEnabled=true;}}
    private Button Button(string title,Func<Task> action){var b=new Button{Content=title,MinHeight=44,Margin=new(4),Padding=new(12,4,12,4)};b.Click+=async(_,_)=>await Run(action);return b;}
