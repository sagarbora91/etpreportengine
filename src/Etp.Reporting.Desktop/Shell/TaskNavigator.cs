...
public sealed partial class TaskNavigator(MainWindow window)
{
    ...
    private UserControl ResolveTaskView(TaskDestination task)
    {
        ...
        { view = window.settingsWorkspace; if (id is "ocr" or "sharing") window.settingsWorkspace.SelectIntegrationTask(id); body = id is "connection" or "settings" ? new int[] {0,1,3,6} : new int[] {3,5}; actions = id is "connection" or "settings" ? new int[] {2} : new int[] {}; }
