...
All implementation work that can be completed deterministically from the repository and supplied ETP exports is built through productisation migration `0014`. The remaining items are business-policy inputs or external-runtime approvals and are explicitly isolated below; the engine does not guess them.
