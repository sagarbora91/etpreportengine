...
Latest committed checkpoint: `08e39c889fe2a13b09aa33ffa788df63f0b800fd`, recorded by `docs/audit/ETP-CLOSURE-SPRINT-CHECKPOINT-2026-08-28.md`. After the scoped Help and backup-safety changes, the release build completed with 0 warnings and 0 errors; the combined .NET suite passed 516/516 tests, the security regression suite passed 23/23 tests, and the .NET/npm audits reported no known vulnerabilities. An unsigned 1.8.4 engineering candidate, SBOM and provenance record were produced from the integrated working tree. Because those changes are not yet a reviewed commit, the 1.8.4 candidate is not presented as clean-commit or production-promotion evidence. This automated evidence does not substitute for clean-PC, live recovery, production hardware, human UAT, source-data, signing or licensing evidence.

## Current verification boundary

- `VERIFIED` rows below are supported by the 28–29 August 2026 checkpoint/baseline evidence and the current 516-test integrated verification, including the focused evidence cited in each row.
...
- `REL-008` has its hashes, SBOM, provenance and no-rebuild record prepared, but remains externally blocked until the exact accepted and signed candidate is promoted. `REL-011` remains pending explicit publication authorization.
...
| `ARCH-001` | One WPF executable with clear Domain, Application, Import, Reporting, SQL and Desktop responsibilities (`SRC-CORE`, `SRC-MOD`) | Dependency tests plus source inspection | `VERIFIED` | One WPF executable remains; dependency guardrails and the combined 512-test suite pass at `08e39c8`. |
...
| `ARCH-003` | One composition root constructs dependencies (`SRC-MOD`, `SRC-LIC`) | Construction limited to App/composition/adapters; no service locator | `VERIFIED` | `DesktopCompositionRoot` constructs the shell and extracted workspaces; the construction inventory is zero and guarded at `08e39c8`. |
| `ARCH-004` | Application use-case contracts are the normal Desktop boundary (`SRC-MOD`) | Desktop workspaces depend on contracts, not SQL/import implementations | `VERIFIED` | Access, Dashboard, Reports, Archive, Daily Workflow, Source Inbox, Imports, Registers, Accounting, Distribution, Operations and Administration use Application contracts with composed SQL adapters. |
...
| `ARCH-007` | No report formulas or workbook parsing in Desktop (`SRC-CORE`, `SRC-MOD`) | Architecture/source tests | `VERIFIED` | Report calculations remain in Reporting/SQL adapters and workbook parsing remains in Import; source guardrails pass. |
| `ARCH-008` | Preserve startup modes during refactor (`SRC-MOD`) | Normal, database-initialize and automation-once tests | `VERIFIED` | Startup coordinator tests cover exact routing, interactive/headless behavior, exit mapping and diagnostics in the combined suite at `08e39c8`. |
| `ARCH-009` | Architecture guardrails prevent regression (`SRC-MOD`) | Automated boundary, construction, route and size/responsibility tests | `VERIFIED` | The focused architecture/composition/ownership/UI-smoke set and combined suite pass at `08e39c8`. |
...
| `IMP-001` | Automatic file/profile/store/business-date detection (`SRC-CORE`, `SRC-DAILY`) | Corpus tests for every active profile | `VERIFIED` | Previous audit claims implementation; exact corpus evidence must be renewed. |
...
| `IMP-003` | Progress, cancellation, retry and clear failure summaries (follow-up item 5) | Large-batch UI automation and atomic cancellation test | `EXTERNAL_VALIDATION_BLOCKED` | The extracted Import workspace owns progress/cancel/retry state; failed validation and source changes clear stale validated snapshots. Large-batch installed revalidation remains. |
...
| `IMP-006` | Unknown layouts fail closed with actionable diagnostics (`SRC-CORE`, `SRC-DAILY`) | Malformed/unknown workbook tests | `VERIFIED` | Profile diagnostics exist; all entry paths must be rechecked. |
...
| `IMP-009` | Active deterministic profiles R003, R013, R022, R025, Variant Stock Ledger and Closing Stock | Per-profile corpus/reconciliation tests | `VERIFIED` | Must be verified individually in closure phase. |
...
| `REP-005` | PDF export for every supported report (follow-up item 10) | Each report exported, parsed and rendered | `VERIFIED` | Format selection/execution is centralized outside MainWindow and focused export tests pass; exact per-report rendered/Unicode matrix remains. |
...
| `OPS-013` | Harden settings storage and local-path validation (follow-up item 19) | ACL/path traversal/network/invalid path tests | `IN_PROGRESS` | Atomic rooted settings, traversal/invalid-path rejection, launch allowlisting, reparse-point checks and Windows-integrated SQL tests pass. Explicit UNC/network-root rejection and Windows ACL ownership verification remain defense-in-depth work when caller-supplied paths are treated as a supported production boundary. |
...
| `REL-005` | Automatic versioning and changelog generation (follow-up item 13) | Clean-tag build evidence | `VERIFIED` | The version/changelog pipeline produced the 1.8.4 build, installer and offline candidate fail-closed; a clean committed rebuild remains part of final promotion. |
...
| `REL-008` | Exact tested installer is the released artifact | Hash, manifest, SBOM and no-rebuild promotion | `EXTERNAL_VALIDATION_BLOCKED` | The 1.8.4 candidate hashes, CycloneDX SBOM, provenance and no-rebuild record are prepared under `verification/release/`; provenance records that it came from an integrated dirty worktree. A reviewed commit, clean rebuild, clean-PC/UAT acceptance, signing, re-hashing and explicit promotion remain open. |
...
| `KNOW-004` | Knowledge reflects accepted architecture/rule changes without duplicating task state (`SRC-KNOW`) | Vault validation and spot audit each phase | `VERIFIED` | The checkpoint vault validation reports 20 notes, 0 broken Wiki links and 0 stale-note warnings; current architecture/rules remain aligned. |
...
This is a current closure classification at the `08e39c8` baseline plus the scoped 29 August 2026 Help and release-evidence work, not a production-approval statement. Active requirement rows now comprise 61 `VERIFIED`, 41 `EXTERNAL_VALIDATION_BLOCKED`, 6 `OWNER_INPUT_BLOCKED`, 2 `SOURCE_DATA_BLOCKED`, 3 `OWNER_APPROVED_DEFERRED`, 1 `IN_PROGRESS` and 1 `NOT_STARTED`; the four legacy rows remain excluded. Settings defense-in-depth, final promotion/publication, external acceptance, Owner decisions, source inputs and deferred licensing remain open as described above.
