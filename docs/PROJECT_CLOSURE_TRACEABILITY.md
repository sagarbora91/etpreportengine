...
| `ARCH-003` | One composition root constructs dependencies (`SRC-MOD`, `SRC-LIC`) | Construction limited to App/composition/adapters; no service locator | `NOT_STARTED` | `MainWindow` remains a temporary composition root. |
...
| `ARCH-008` | Preserve startup modes during refactor (`SRC-MOD`) | Normal, database-initialize and automation-once tests | `IMPLEMENTED_NOT_VERIFIED` | Current startup modes exist; no post-refactor closure evidence yet. |
| `ARCH-009` | Architecture guardrails prevent regression (`SRC-MOD`) | Automated boundary, construction, route and size/responsibility tests | `IN_PROGRESS` | DESK-009/010 tests exist; construction and module ownership gates remain. |
