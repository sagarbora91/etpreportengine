...
Closing waited on four things, set out in the closure record's section 8. **One of them is now done**: the four small product fixes, later on 25 September — `docs/OPERATIONS.md` corrected in all three ways, backup receipts given a `purpose` so rotation can never delete setup's own pre-migration copy (or a pre-rollback one, which had the identical hazard), an edition that encrypts refused in words instead of failing on a missing file, and the administration screen made to show why a save failed. That last one was bigger than it had been written up as: **four of its five tasks** hid the message, not only Users. Reviewing those fixes then turned up a fifteenth defect, older than them and fixed the same day (item 15): a backup folder that had filled up could never empty itself, because rotation ran only after a successful backup while the free-space check refused before one started. Each is recorded in section 6 with how it was tested, and section 8 has a dated addendum.

**Three remain, and all three are Sagar's**: the **shop PC install** (the only machine where the Owner is a SQL administrator only through `BUILTIN\Administrators`, and so the real test of the drill-task fix); his rulings on **A4.7 signing, D9 backup encryption and A4.4's row counts**, each a contradiction between the plan and the 17 September decisions; and capturing the **SQL backup folder's permissions** on both machines (28 `.bak` files, three with real customer data, never checked). Two smaller questions are waiting on him too, both from the fixes: whether safety backups should be capped automatically now that they accumulate one per upgrade, and the fact that the encrypted-edition refusal is tested but has never run on an edition that encrypts, because both live machines are Express.
