...
- **Running when saved — check their results first:**
  ...
  2. Background task `task_b380b90b` "Stop tests writing to the owner's diagnostics log": **finished by ~19:06 IST** per its memory note — `DesktopDiagnostics` honours `ETP_DIAGNOSTICS_DIRECTORY` (full paths only) and `tests-dotnet/TestSupport/DiagnosticsIsolation.cs` (module initializer) sends each test run to `%TEMP%\EtpTestDiagnostics_<guid>`; a full run left the Owner's log byte-identical. **Uncommitted in this worktree**; it changes app code, so it belongs in the installer.
  3. **Finished 22 Sep ~19:50, all four verified "safe to hand to Sagar", committed.** Findings to act on: (a) setup's own pre-migration backup is deleted by the same day's Daily Backup rotation (newest per UTC day), so the runbook makes an independent backup to the SQL Backup folder mandatory (step A6) — product fix later: keep pre-migration backups out of rotation; (b) Windows PowerShell's execution policy on this PC is the default Restricted, so `& 'script.ps1'` is refused — the runbook uses a tested form; `docs/OPERATIONS.md` still shows `&` and needs the same fix; (c) Settings > Users does not show why a save failed (status line not in that view) — product fix later; (d) the VM's SQL may have been upgraded to Developer Edition on 17 Sep (PHASE-4-AUDIT section 11): if so its backups encrypt and need exported recovery keys first — runbook step B6 checks; (e) D14 recommendation: keep Windows' Segoe UI (Microsoft's licence forbids bundling it) and replace the misleading NullReferenceException when it is missing with a clear error (~half a day); (f) EtpReportingHelios recommendation: verified backup, then drop, after the checks in its document; (g) the agents' read-only queries made SQL's automatic statistics create 5 column statistics on EtpReporting (no data or definition changed; recorded in the Helios document). Workflow `wf_55b339dd-669` (4 prepare agents + 4 verifiers) wrote, in this worktree: `docs/audit/LIVE-INSTALL-RUNBOOK-2026-09-22.md`, `docs/audit/VM-AND-SECOND-MACHINE-RUNBOOK-2026-09-22.md`, `docs/audit/ETPREPORTINGHELIOS-FINDINGS-2026-09-22.md`, `docs/audit/D14-FONT-DECISION-2026-09-22.md`, and the read-only check script `scratchpad/recovery/live-verify.ps1`. If it was cut off, resume it: `Workflow({scriptPath: "C:\Users\Sagar\.claude\projects\C--Codex-Reporting-Manger-opus-recovery\38f855f7-3921-4259-b901-2a97df36d8cc\workflows\scripts\phase4-live-prep-wf_55b339dd-669.js", resumeFromRunId: "wf_55b339dd-669"})`. Review its four documents before handing them to Sagar, then commit them.
- **Careful:** both background tasks are editing THIS worktree, not separate ones. When saved, uncommitted changes from them were in `src/Etp.Reporting.Desktop/DesktopDiagnostics.cs`, `Controls/TablePresentation.cs`, new `Controls/PropertyChangeWatcher.cs`, `MainWindow.Shell.cs`, `tests-dotnet/TestSupport/` (new), `Etp.Reporting.Desktop.Tests/{DesktopDiagnosticsTests,Phase3ShellTests,PropertyChangeWatcherTests}.cs` and csproj, `Etp.Reporting.SqlServer.IntegrationTests/{DurableImportHistoryTests,DurableImportProblemsTests,HeadlessStartupFailureTests}.cs` and csproj. Do not commit, stash or revert those until both tasks report; then review them as their work, run the full gate, and commit them separately from anything else.
