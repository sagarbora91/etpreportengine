...
**14 September checkpoint: INCOMPLETE.** [Current handoff](ETP-SESSION-HANDOFF-2026-09-14-UI-REDESIGN.md) supersedes old access/publication statements: bdba5a7 is pushed with portable artifacts; r11 is the last verified installed candidate; worker was CLOSED on the latest check. The installed hash/data-preservation subset passes, but visible launch and full UX/J requirements remain UNVERIFIED. Settings/caption reports remain unreproduced; the supplied overview shows the pending `1 tasks` wording defect. No matrix result is upgraded by this documentation update. The following r9 evidence is historical and scoped to its recorded candidate.
