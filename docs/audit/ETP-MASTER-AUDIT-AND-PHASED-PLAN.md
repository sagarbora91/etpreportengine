...
| D4 | **DECIDED 15 Sep 2026:** Both. Codex lists every distinct BRAND/BRANDNAME/CLUSTER from the real exports with a proposed DSR row in `PHASE-2-REPORT.md` and Sagar ticks each; AND Phase 2 ships an Owner-editable **Brand rows** master in Settings (store, DSR row label, order, mapped ETP brand codes) so rows can change later without code. Rows: Titan = NEBULA, EDGE, XYLYS, AUTOMATIC, RAGA; Helios = SEIKO, CITIZEN, CERRUTI. **Still pending 16 Sep 2026.** `0024_evening_report_masters.sql:23-25` seeds these eight labels and self-maps them by exact label, so five can never match the source as written: the data spells it **CERUTI** (one R); **NEBULA** appears only on a packaging line; **AUTOMATIC/SEIKO/CITIZEN** exist only at cluster level (**GAUTO/SEKOG/CTZNG**). Verified live: Helios DSR shows CERRUTI 0.00 with all value under Other / unmapped. `0024` is immutable, so approval must land via the Settings brand editor or a new migration. | Phase 2 | — |
...
- A1.2 SQL check: Titan 25 Aug lines sum NETAMOUNT = 34,215.00 and NETVALUE = 28,995.76; Helios 29,290.00 / 24,822.02; August MTD Titan NETAMOUNT = 938,197 and invoices = 178; Helios 774,869 / 37 (values recomputed by Claude from the workbooks; corrected 16 Sep 2026 from 182/38, which counted SR/BC documents that task 1 excludes from the denominator).
...
- A1.9a Importing the two-year Helios R025 stores exactly 790 lines and 759 invoices (workbook counts) with zero CONFLICT outcomes; invoice 100000068 exists three times with FY-end years 2025, 2026 and 2027; every month in `golden-monthly-HEMW-R025.csv` matches. The two-year `R030_Variant_Stock_Ledger.xlsx` (3,955 rows, nine transfer types) imports without a blocker.
...
6. **CRO-wise sales** — target, net sales, net qty, discount, ATV, AUPT, unique invoice count, with monthly targets from Phase 1. **Confirmed by Sagar 16 Sep 2026:** ATV = CRO net sales / CRO unique invoices; AUPT = CRO net quantity / CRO unique invoices. The denominator is distinct INV documents keyed by (store, financial-year, document number) — SR and BC are excluded from it while their signed values remain in the numerators; multiple product lines on one invoice count once; a CRO with no INV invoice shows no ratio rather than a zero.
...
8. **Feedback.** Status and result messages wrap (max 3 lines, "More" opens details). **Amended 16 Sep 2026 (Claude, under Sagar’s instruction to finish without further input; reverse if unwanted):** the three-line wrap applies to toasts and inline result areas, which already honour it. The fixed 28 px status footer of task 2 shows **one line plus a hidden-line count plus "More details"** instead, because three lines there cost about 24 DIP of chrome and would drop the content area below the 610 px floor on the real maximized window (622 DIP measured, falling to about 598). No message text is lost either way. The two rules were in direct conflict; this resolves it in favour of the content budget; required fields validated in the presentation session before any database call with a visible "*"; `DesktopFriendlyError` never shows "(Parameter …)" or RAISERROR text; success toast; progress bar plus Cancel for import, export, pack generation and finalise; confirmation sheet before Finalise, Reopen, Waive, Reject, Cancel batch, Run backup/drill/automation.
