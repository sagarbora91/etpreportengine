...
## 10. Closure candidate re-audit — 17 September 2026

Branch `phase-2-3/closure`, HEAD **`dfd9f8c`**, based on `0a0c38f`. Build and full-suite results, migration immutability and the shared evidence are recorded in `PHASE-2-AUDIT.md` §11: **857 passed, 0 failed, 3 skipped**, both builds clean, reproduced independently with `-m:1 -nodeReuse:false`.

### Job 4 — Problems → Retry failed — **PASS**

The control exists and is reachable by touch: `ImportProblemsView.cs` adds a **Retry failed** button beside the status filter with `MinHeight = 44`, an automation name and help text naming Ctrl+R, which is retained as an accelerator.

**Only failed files are retried.** `FolderImportService.RetryFailedAsync` runs `FailedPaths` alone, recomputed after every run. `MergeRetryResults` (`ImportWorkspaceView.xaml.cs:129-135`) replaces an entry only where the previous result `.Failed` **and** its `SourcePath` matches a retried path; everything else passes through untouched. `RunImportAsync` skips the `latestResults = []` reset when `retry` is true, so earlier successes survive.

A detail worth crediting: retry preserves the **sibling folder scope** captured in `detectedScopes`, so an empty export that depends on other exports in its folder still resolves when retried alone. Retrying one file in isolation would otherwise lose that context.

**Permissions and busy state.** The click handler re-checks `CanRetry`; `RunImportAsync` re-checks `accessProvider().CanImport` and throws `UnauthorizedAccessException`; a restatement additionally requires `CanAdminister`; `BeginImportOperation()` returns null while an import is in flight, so retry cannot run concurrently; the button follows `RetryAvailabilityChanged` and is re-enabled in a `finally`.

`ImportRetryClosureTests` imports a real sanitised R025 workbook beside a deliberately corrupt file, repairs the input, raises the **actual WPF button event**, and asserts only the failed path was re-read while the successful R025 file, its facts and its row outcomes are unchanged. Owner and Manager can retry; Viewer's button is disabled, direct invocation is rejected, and SQL denies the write.

**This is automated button-path evidence, not physical touch acceptance** — Codex states this itself, and this audit does not claim otherwise.

### A3.3 — 125% DPI — **NOT VERIFIED**

I attempted this and it did not produce valid evidence. Reporting what happened rather than what I hoped for.

The acceptance VM now runs a build that **matches this candidate exactly**: `1.8.8+dfd9f8c49a835f3cc2fc43ffba363e7372c3f74d`, 28 migrations, transferred by `Copy-VMFile` and confirmed by SHA-256 (`B7D717C4…`) on both sides. It previously held `0a0c38f` — the base, not the candidate — so testing it before replacing the build would have measured the wrong code. That check was worth doing.

A disposable `EtpDpi` database was initialised by the candidate (exit 0, 28 migrations) and the app was driven maximised in the guest's interactive session with UI Automation. But:

- `SystemParametersInfo(SPI_SETLOGICALDPIOVERRIDE)` **did not change the guest's scaling**: system DPI read 96 before, 96 after, and the window reported DPI 96 (scale 1.0).
- The guest console is at **1024×768**, not the 1366×768 the criterion specifies.

So that run satisfied **neither** dimension of A3.3, and its numbers are not offered as a DPI result. For the record, at 100% and a 1372×600 client the layout behaved: one 24 DIP scrollbar and exactly two controls under 44 DIP, both 17 DIP scrollbar arrow buttons, which fall under the plan's own 24 px scrollbar exception.

**125% is now primed in the guest** — `HKCU\Control Panel\Desktop\LogPixels = 120`, `Win8DpiScaling = 1` — and applies at the next guest sign-in. Completing this needs two things I should not do unilaterally: the VM console reconnected at **1366×768** (Enhanced Session Mode is now enabled, so the resolution can be chosen on connect), and one **sign-out and sign-in inside the guest**. I will re-run the measurement immediately afterwards.

The residual risk is low but genuinely unproven: the application declares PerMonitorV2 awareness, every interactive control measured at or above 44 DIP, and the 816×480 half of A3.3 passed earlier. None of that is evidence for 125%.

### A3.8 — timed touch walkthrough — **NOT VERIFIED (manual acceptance)**

Cannot be produced by any automated means. A walkthrough has been prepared for Sagar covering the full evening — import, Problems → **Retry failed**, History, walk-ins, cash, DSR, Export PDF — finger-only and timed, with a note that step 4 requires a genuinely failed file and should be skipped and declared rather than manufactured on the shop PC. It also asks him to switch the shop PC to **Touch** density first, since it is currently on Desktop.

### Acceptance position

| ID | Status | Basis |
|---|---|---|
| A3.1 | **PASS** | Landing and two-tap routes unchanged and re-verified earlier |
| A3.2 | **PASS** | Measured natively: 0 scrollbars on all six named screens |
| A3.3 | **PARTIAL** | 816×480 half passes; **125% DPI NOT VERIFIED** — see above |
| A3.4 | **PASS** | No interactive control under 44 DIP except scrollbar arrows, within the plan's exception |
| A3.5 | **PASS** | Typed dates retain `dd MMM yyyy`, verified natively |
| A3.6 | **PASS** | "Enter the walk-in count" inline, zero technical strings |
| A3.7 | **PASS on intent** | Grep hits are route ids and one search keyword; nothing banned is rendered |
| A3.8 | **NOT VERIFIED** | Manual acceptance, walkthrough prepared |
