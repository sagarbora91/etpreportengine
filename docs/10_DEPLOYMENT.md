...
The installer presents a selected-once `sqlprerequisites` task that controls only whether missing Microsoft SQL Server 2022 Express and `sqlcmd` packages may be installed with `winget` under Microsoft's licence terms. An operator may clear that task when approved offline media or managed software deployment supplies the prerequisites. Clearing it passes `-SkipSqlInstallation`; it does **not** skip database preflight, backup, migration, health validation, or operational-task configuration. The elevated [bootstrap script](../scripts/bootstrap-etp-prerequisites.ps1) runs after application files are installed on every clean install, repair, and upgrade. Any failure terminates setup with an actionable pointer to `%ProgramData%\EtpReporting\SetupLogs` and prevents the installer from offering normal application launch.
...
3. Acquire the session-owned SQL application lock so two clients cannot plan or apply migrations concurrently.
...
The current installer unconditionally runs the bootstrap and the application once with `--initialize-database`; the optional task controls prerequisite package installation only. The bootstrap validates SQL Server/database compatibility and bundled migration availability. When an existing database has pending migrations, it calculates a capacity floor, creates a collision-resistant `COPY_ONLY`/`CHECKSUM` backup, verifies it with `RESTORE VERIFYONLY`, and validates the receipt path, size, and SHA-256 before migration. The headless application path creates the database when absent, rejects missing or checksum-changed applied migrations, and applies each pending migration and journal entry transactionally. The migration runner acquires a bounded, session-owned exclusive SQL application lock before discovery/planning and retains it through every migration commit; lock timeout, cancellation, or acquisition failure stops before planning. Setup succeeds only after the database is online/read-write, the journal count exactly matches the bundled migrations, and `DBCC CHECKDB` passes. Failure preserves the database and verified backup for diagnosis without automatic reverse migration, restore, or deletion. This chain is implemented in source; compiled-installer, live concurrent-upgrade/failure, and restore-drill evidence remain external release gates.
...
After upgrade, run connectivity, migration, import-lineage, and representative report smoke checks. The [installer lifecycle test](../scripts/test-installer-lifecycle.ps1) must run elevated on a disposable target machine. It exercises silent installation, same-version repair or a supplied prior-version upgrade, mandatory database initialization/health validation, and uninstall in a temporary application directory. By default it disables only automatic prerequisite-package installation, so SQL Server 2022 Express and `sqlcmd` must already be present; `-AllowPrerequisiteInstallation` tests the separate package-install path. Do not run it against a production database. Application-binary rollback is allowed only when the older version is compatible with the upgraded schema. The [rollback helper](../scripts/invoke-release-rollback.ps1) takes a verified backup unless explicitly overridden and reinstalls a supplied prior installer while retaining SQL data; operators must still verify schema compatibility and controls. Database rollback means restoring the verified pre-upgrade backup and reconciling any work performed after it; down-migrations are not assumed. The release runbook must define the business outage/data-reentry decision.
...
Operational logs should include application/release version, correlation/import batch ID, profile and migration versions, event severity, and safe error classifications. They must exclude passwords, full connection strings, unnecessary source cells, and personal data. Desktop diagnostics are stored under `%LocalAppData%\EtpReporting\Logs` as JSON Lines with privacy-safe event/source tokens, correlation ID, exception type/HResult and application version only; exception messages, stack traces and business/source values are excluded. Each segment is limited to 5 MiB, at most 24 segments are retained, and segments older than 180 days are pruned. If pruning or rollover cannot preserve those bounds, diagnostics skip the new write without failing application startup or the user operation.
...
The current [GitHub Actions workflow](../.github/workflows/ci.yml) runs on ordinary pushes and pull requests, restores/builds/tests the solution in Release mode on Windows, runs the 23 privacy/security regressions, and enforces the fail-closed [dependency security scan](../scripts/invoke-security-scan.ps1) with uploaded JSON evidence. Versioning and changelog preparation are supported by [the release-version script](../scripts/set-release-version.ps1); a versioned offline ZIP can be assembled from an already-built installer by [the offline-package script](../scripts/new-offline-deployment-package.ps1). This CI result still does not constitute installer, live SQL, signing, SBOM/provenance or external-acceptance evidence. A production release pipeline must additionally:
