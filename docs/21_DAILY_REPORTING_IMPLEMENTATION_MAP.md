...
| Deployment | SQL bootstrap installer, migrations, backups, health, recovery and automated-operation tasks | installer and PowerShell scripts | Operational | Forward migrations through `0013` deploy through the checksum-controlled bootstrap; Phase 2 adds background database access plus guards against Store Manager changes to users, masters, migrations and append-only audit data. |
