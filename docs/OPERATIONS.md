...
**D9 revised: native SQL backup encryption using AES-256 and `EtpBackupCert` where the edition supports it, and a recorded unencrypted backup where it does not.** SQL Server Express and Web refuse `BACKUP ... WITH ENCRYPTION`. On those editions the backup is still taken, verified and checksummed, and its receipt records `"encryption": "NONE"` rather than claiming AES-256; the completion message and the audit entry both say plainly that the file is not encrypted. **An unencrypted backup file must be protected at rest by the folder policy, because nothing in the file itself protects it.** The edition probe fails closed: an answer it cannot interpret refuses the backup rather than silently producing an unprotected one on an edition that could have encrypted it. A supported edition remains the better choice and is what an encrypted backup requires. [Microsoft backup encryption documentation](https://learn.microsoft.com/en-us/sql/relational-databases/backup-restore/backup-encryption?view=sql-server-ver17).
...
| Folder access | The selected dedicated non-admin account is `<machine>\EtpAutomation`; elevated provisioning and before/after ACL verification are still required. See below. |
| Release signing | **A4.3 decided: unsigned releases are accepted for now.** The build scripts sign when given a certificate and skip signing when not; supplying one credential without the other still fails, so unsigned is a deliberate choice rather than a typo. An unsigned install means Windows warns on install and the payload cannot be traced to its publisher. Acquiring a signing certificate remains open. |
| Recovery custody | Sagar chooses two genuinely off-PC locations and a separate password custodian. |
| Recovery evidence | Complete a backup, a same-machine drill, and a second-machine restore. On a supported edition the backup is encrypted and the second-machine restore needs the exported certificate; on Express or Web it is unencrypted and needs none. Native encrypted restore and second-machine recovery have not been verified by this coding task. |
...
By default, `Backups`, `Documents`, `Share` and `SetupLogs` beneath `%ProgramData%\EtpReporting` allow SYSTEM and Administrators full control, and the SQL service and dedicated local `EtpAutomation` account Modify. The parent `EtpReporting` folder and `Operations` give the SQL service and automation account read access. The parent is protected too, so a broad parent grant cannot be used to replace a protected child. Inheritance from broad Windows groups is removed.

On 16 September 2026 Sagar delegated the automation-account choice; the selected account is `<machine>\EtpAutomation`, with no Administrators membership. Folder access for this dedicated account is now the default, and setup writes the protected machine configuration. `-CreateAutomationAccount` provisions a missing account with an unrecorded random password for S4U tasks. `-GrantAutomationFolderAccess:$false` retains strict three-identity protection for installations that explicitly want unattended operations unconfigured. Neither mode grants the shared staff login access. Staff document/share workflows still need a separately approved access design; do not grant `BUILTIN\Users` access as a shortcut.
...
1. **Prepare the SQL instance and recovery storage.** Preinstall a SQL Server 2022-or-newer instance, start it, and install protected Microsoft Sqlcmd. An edition that supports native encrypted backups (Standard, Developer or Enterprise) is required for an encrypted backup; Express and Web are accepted and produce a recorded unencrypted backup instead, which then depends entirely on the folder policy for protection at rest. Record the instance, database, real service identity, version and edition. Bootstrap does not purchase or install a compatible SQL edition.
2. **Prepare the release.** Signing is optional per A4.3: supply both `-CertificateThumbprint` and `-TimestampServer` to sign, or neither to build an unsigned release. Supplying only one fails rather than degrading silently to unsigned. The signing certificate must have its private key and code-signing purpose in the build operator's Current User certificate store. `build-windows-release.ps1` signs the published executable and packaged PowerShell scripts; `build-windows-installer.ps1` signs the resulting installer and writes its SHA-256 file. Signing requires a valid timestamp, and the installer builder verifies payload signatures even with `-SkipReleaseBuild`. Preserve the release receipt and hashes. Verify the publisher and signature on the installed executable, all packaged scripts, and installer. Deployed PowerShell runs with `RemoteSigned`, not `AllSigned`: `AllSigned` on an unsigned install refuses the nightly backup, the monthly drill and every owner-started maintenance operation, which the shop would discover only when a restore was needed. The script allow-list and `Assert-EtpProtectedInstall` still refuse any script a non-administrator can edit, so this does not widen what can run. Do not switch to `Bypass`.
3. **Prepare the accounts and folder policy.** Create the approved dedicated local account. Run the signed folder setup from the protected installation as Windows administrator. The following example applies strict permissions only:
   ...
   & 'C:\Program Files\Saagar Traders\ETP Reporting Engine\scripts\initialize-etp-operation-folders.ps1' `
       ...
       -ServerInstance '.\ETP' -Database 'EtpReporting' -CreateAutomationAccount
   ```

   Use the actual SQL service identity. The automation account defaults to the local machine's `EtpAutomation`; override with `-AutomationPrincipal` only for a separately selected dedicated non-admin account. Verify the resulting permissions and protected configuration. Strict mode alone cannot satisfy the unattended setup prerequisites.
...
7. **Install the restricted SQL operations module.** P4-13 (fixed 22 September 2026): the module no longer needs a database master key, so it installs on Express and Web as well. `EtpBackupCert` is required only on an edition that encrypts backups. Before running it, apply every bundled migration, including `0032` (P4-14: before it, Settings → Users left each Store Manager unable to connect), and add the dedicated account as an active Store Manager in Settings → Users. Then run the script elevated, as a SQL administrator:
   ...
   Use the standard protected backup folder unless a separately reviewed deployment accounts for every UI and task default. If a prerequisite is missing, the script names the step that comes first. A failed run removes the signer it created and keeps the broker: an administrator, and so setup's pre-migration backup, can still use it, and the dedicated account cannot until the module is reinstalled. Verify the module under the dedicated account before enabling unattended operation.
8. **Exercise backup and recovery, then install tasks.** Under the actual dedicated account, run `backup-etp-database.ps1` without target overrides so it uses the common configuration. Then run `invoke-etp-recovery-drill.ps1` as the Owner, elevated. The drill restores a full copy and runs `DBCC CHECKDB` on it, which SQL Server allows only a SQL administrator to do (P4-15, decided 22 September 2026: the drill runs as the Owner). The dedicated account is refused with that explanation. Complete the second-machine exercise below. When these succeed, run the three signed `install-*-task.ps1` scripts as administrator. `install-monthly-recovery-drill-task.ps1` keeps the account the drill already runs as, or, on a first install, uses the Owner running setup; `-DrillPrincipal` chooses another. It refuses a service account, or an account that is not a SQL administrator, including through a Windows group such as `BUILTIN\Administrators`. A fully prepared installation can run `bootstrap-etp-prerequisites.ps1 -ApplicationDirectory <protected-installation>` to perform its configured preflight, any needed verified pre-migration backup, migration/integrity checks and task installation. It requires the protected configuration and preinstalled prerequisites; it is not an account/certificate provisioning wizard.
9. **Verify before handover.** Confirm Daily Backup and Automated Operations run as the dedicated account at Limited run level, the Monthly Recovery Drill as the Owner at Highest, and none as SYSTEM; validate actual task results and a daily backup less than 24 hours old. Confirm Viewer day-reopen/audit deletion fail and Store Manager fact edits/superseding fail. Inspect the three required folder ACLs for any `BUILTIN\Users` access. Check system status, support output privacy, and the saved interactive target under both Owner and staff logins.

### SQL operations module

`install-etp-sql-operations.ps1` installs a database-specific `etp_operations_<hash>` procedure in `master`, with the database and backup/restore roots fixed at installation. Calls accept only the supported operation and a constrained backup filename. The procedure verifies the caller's active application role. Backups and metadata checks are open to the dedicated account. The drill is refused to anyone but a SQL administrator; it creates an isolated `EtpRecovery_<guid>` database and refuses the backup if the restored copy has TRUSTWORTHY or cross-database ownership chaining on.

Each module has its own signer, `EtpOperationsModuleSigner_<hash>`: a certificate and login that give that one procedure `CREATE ANY DATABASE` and `VIEW SERVER STATE`, which reading a backup's contents and refusing to overwrite a file require. No database master key is involved. The certificate is protected by a password generated inside SQL Server and never recorded, and its private key is removed as soon as the procedure is signed. The signature still verifies, but nothing can be signed with that certificate again; reinstalling creates a new one. It is not the backup-encryption certificate and is not the Windows task login. Earlier builds used one shared `EtpOperationsModuleSigner` with a live private key and `ALTER ANY DATABASE`; the installer removes it once nothing is signed with it. Altering the module requires a reinstall. Do not give the automation account `sysadmin` or `dbcreator` to resolve a failed drill: the drill is the Owner's.

After the drill, its result is recorded as the dedicated account's database user. The recording procedures live in the application database, where any application Owner can change them, so they never run with the drill's administrator rights. The recording is refused if the database is marked TRUSTWORTHY.

The installer grants the dedicated account module execution, backup-certificate visibility where the edition encrypts, `db_backupoperator` on the application database, and `etp_automation` for verified status recording. Reprovisioning an application account removes its automation and backup-operator memberships. After an approved account change, review and reapply the dedicated setup explicitly. The master module also checks active application role at each call; remove its old direct grant when retiring an automation identity.
...
Daily Backup and Automated Operations run as the dedicated account at Limited run level. The recovery drill runs as the Owner at Highest run level: a UAC-filtered token loses `BUILTIN\Administrators`, so an Owner who is a SQL administrator only through that group would lose it inside the task. All use S4U logon, StartWhenAvailable, IgnoreNew for overlap, and a two-hour execution limit. The monthly runner's day check means a missed run caught up on another day does not perform the missed drill; run a manual drill and inspect its evidence. Task scheduling does not prove that a job succeeded.

A backup uses a unique filename and SQL `COPY_ONLY`, `CHECKSUM`, `RESTORE VERIFYONLY`, and AES-256 encryption on an edition that supports it. On Express and Web the encryption clause is omitted and the receipt records `NONE`. After SQL verification, the script writes `<backup>.receipt.json` and `<database>-latest-verified.json`, containing the exact path, length, SHA-256, certificate reference and backup file metadata. These files contain paths and belong in the protected backup folder, not a general support attachment.
...
| Script/signature or installation-permission failure | Verify the protected installation ACLs, and the publisher and signatures if the release was signed. Repair or reinstall the package; do not weaken the execution policy below `RemoteSigned` or bypass `Assert-EtpProtectedInstall`. |
