...
7. **Install the restricted SQL operations module.** **Known defect P4-13, open as of 17 September 2026: this step cannot succeed on SQL Server Express or Web.** `install-etp-sql-operations.ps1` still requires `##MS_DatabaseMasterKey##` and grants `VIEW DEFINITION` on `EtpBackupCert`, and reports `Create the backup certificate first.` — but the D9 revision means those editions never encrypt a backup and need no such certificate. Module signing genuinely needs a master key and the `EtpOperationsModuleSigner` certificate; backup encryption is what does not apply. Until this is separated, the module, and therefore the verified backup and the recovery drill, cannot be produced on Express. See `docs/audit/SESSION-HANDOFF-2026-09-17.md`. With the certificate present, Phase 4 migrations applied, and the dedicated account active as Store Manager, run the signed script as SQL administrator:
