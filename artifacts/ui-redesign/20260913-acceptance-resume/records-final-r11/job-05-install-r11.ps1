...
$receipt=Get-Content -LiteralPath (Join-Path $localRoot 'candidate-r11-receipt.json') -Raw | ConvertFrom-Json
$installer=Join-Path $localRoot 'installer-1.8.8-r11\EtpReportingEngine-Setup-1.8.8-x64.exe'
...
Invoke-Command -Session $Session -ScriptBlock {
    ...
    $root='C:\ETPAcceptance\UI-20260913-r11'
    if(Test-Path -LiteralPath $root){throw 'Candidate install attempt already exists; inspect it before retrying'}
    New-Item -ItemType Directory -Path $root | Out-Null
}
Copy-Item -ToSession $Session -LiteralPath $installer -Destination 'C:\ETPAcceptance\UI-20260913-r11\EtpReportingEngine-Setup-1.8.8-x64.exe'
try {
    $result=Invoke-Command -Session $Session -ArgumentList $receipt.artifacts.installer.sha256,$receipt.artifacts.executable.sha256 -ScriptBlock {
        ...
        $result=[ordered]@{timestampUtc=[DateTime]::UtcNow.ToString('o');candidate='1.8.8-r11';version=(Get-Item -LiteralPath $app).VersionInfo.ProductVersion;installedSha256=$exeHash;installerSha256=$installerHash;installerExitCode=$setup.ExitCode;countsUnchanged=$true;settingsUnchanged=$true;integrityPassed=$true;startMenuShortcut=$shortcut;startMenuTarget=$link.TargetPath;uiInteraction='UNVERIFIED - native capture blocked';root=$root}
        $result | ConvertTo-Json | Set-Content "$root\result.json"
        [pscustomobject]$result
    }
    $result | ConvertTo-Json | Set-Content (Join-Path $localRoot 'vm-r11-install-result.json')
} finally {
    Copy-Item -FromSession $Session -LiteralPath 'C:\ETPAcceptance\UI-20260913-r11' -Destination (Join-Path $localRoot 'vm-r11-install-evidence') -Recurse
}
